...
router.get('/', async (req, res) => {
  ...
  if (res.locals.user) {
    ...
    res.render('portfolios/portfolio.ejs', {
      message: `You need to login to create a new portfolio.`,
      portfolio: null
    })
  }
})
...
router.get('/to', async (req, res) => {
  ...
  if (res.locals.user) {
    try {
      ...
      console.log(err)
      res.render('portfolios/addstock.ejs', {
        message: `Sorry something went wrong. Please contact your administrator.`,
        portfolio: null
      })
    }
  } else {
...
router.post('/add_stock', async (req, res) => {
  if (res.locals.user) {
    ...
    try {
      ...
      await addStock.save()
      res.redirect(`/portfolio/details/${req.body.portfolioid}`)
    } catch (err) {
  ...
  try {
    ...
    for (let i = 0; i < objPortfolio.portfoliodetails.length; i++) {
      ...
      for (let j = 0; j < objPortfolio.usertransactions.length; j++) {
        if (
          objPortfolio.usertransactions[j].symbol ==
          objPortfolio.portfoliodetails[i].dataValues.symbol
        ) {
          count += 1
...
router.delete('/:id', async (req, res) => {
  if (res.locals.user) {
    ...
    res.render('portfolios/add.ejs', {
      message: `You need to login to view your portfolio.`
    })
  }
})
